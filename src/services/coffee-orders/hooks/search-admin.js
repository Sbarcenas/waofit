  ...
  return async (context) => {
    ...
    if (context.params.query.data_value && user.role == "admin") {
      ...
      const ordersIds = await context.app
        ...
        .innerJoin("users", "users.id", "=", "coffee_orders.user_id")
        .innerJoin(
          "orders_status",
          "orders_status.id",
          "=",
          "coffee_orders.order_status_id"
        )
        .orWhere("users.last_name", "LIKE", `%${value}%`)
        .orWhere("users.first_name", "LIKE", `%${value}%`)
        .orWhere("users.email", "LIKE", `%${value}%`)
        .orWhere("users.phone", "LIKE", `%${value}%`)
        .orWhere("coffee_orders.total_price", "LIKE", `%${value}%`)
        .orWhere("coffee_orders.shipping_cost", "LIKE", `%${value}%`)
        .orWhere("coffee_orders.order_id", "LIKE", `%${value}%`)
        .where({ "coffee_orders.deletedAt": null })
