  ...
  return async (context) => {
    ...
    if (context.params.query.data_value && user.role == "admin") {
      ...
      const ordersIds = await context.app
        ...
        .getModel()
        .query()
        .select("coffee_shop_products.id")
        .innerJoin(
        ...
        .orWhere("coffee_shop_categories.name", "LIKE", `%${value}%`)
        .orWhere("coffee_shop_products.name", "LIKE", `%${value}%`)
        .where({ "coffee_shop_products.deletedAt": null })
